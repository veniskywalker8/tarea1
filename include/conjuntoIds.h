/*
  Módulo de definición de 'conjuntoIds'.

  La estructura de tipo conjuntoIds es un conjunto acotado que almacena identificadores de tio nat.
...
// La función es Theta(n) peor caso, siendo n la cantidad máxima de elementos del conjunto.
TConjuntoIds crearTConjuntoIds(nat cantMax);

// Función para determinar si un conjunto de ids es vacío.
// Retorna true si el conjunto es vacío, false en otro caso.
...
// Procedimiento para borrar el "id" del conjunto de ids "c".
...
// Retorna true si pertenece, false en otro caso.
// La función es Theta(1) peor caso.
bool perteneceTConjuntoIds(nat id, TConjuntoIds c);

// Función que retorna la cantidad actual de elementos del conjunto de ids "c".
// La función es Theta(1) peor caso.
...
// PRE: Ambos conjuntos tienen la misma cantidad máxima de elementos, "n".
